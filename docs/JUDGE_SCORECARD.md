...
| Demo | 20 | 15 | Deterministic local flow, mock build, browser walkthrough, and 180-second visual artifact | Human narration/replay is still required; public deployment is intentionally waived for this demo-only scope |
| **Total** | **100** | **73** | **Local gates, recorded read-only live smoke, and visual artifact PASS; human/external items remain open** | **22 points remain to the 95 target** |
...
| The exposed server boundary is bounded, source-safe, endpoint-allowlisted, and fail-closed under missing credentials or malformed input. | `server/logic.mjs`, `tests/server-audit.test.mjs`, `npm run smoke:server` | 5 / 6 — `PASS [automated]` | Local hardening passes; public deployment is waived for this local demo. |
...
3. `+4` — record and replay the exact three-minute script against a freshly built mock/live artifact; mark any unavailable provider explicitly.
4. `+3` — attach a fresh Atlas/research run plus an outcome/novelty validation artifact (route coverage, response shape, or pilot metric). The fresh smoke is already recorded; only the human outcome/novelty judgment remains. This does not authorize inventing a probability.
...
当前保守基线为 **73/100**，距离 95 分目标 **22 分**。公网部署按用户要求标记为 `WAIVED`，不再作为本地 Demo 的硬阻塞；真实 Atlas 航班动态/履约、Qoder 截图与正式视频人工验收仍保留为 `BLOCKED` 或 `HUMAN`，不会因为 README 或脚本存在而伪造通过。
