...
| --- | --- | --- |
| Real-time flight status 实时航班动态 | The delay input is a labelled simulation; Atlas provides real-time search and a schedule-change webhook contract, but its current post-booking scope does not provide generic Flight Status Inquiry or Confirm Protected Flight. 延误输入是明确标注的模拟；Atlas 有实时搜索和航变 webhook 契约，但当前售后能力不包含通用航班状态查询或确认保护。 | If authorized Atlas Email Service/API monitoring and an `order.schedulechange` callback become available, map that event into the existing `delayMinutes`/event contract. Otherwise integrate a separate authorized flight-status source; never infer live status from `search.do`. 如获得 Atlas Email Service/API 监控与 `order.schedulechange` 回调权限，可将事件映射到现有 `delayMinutes`/事件契约；否则接入独立授权状态源，绝不从 `search.do` 推断实时状态。 |
