...
| 0:35–1:05 | 选任一张，点 `Ask agent which itinerary to choose` | “Agent 先检索官方规则和公开流程，再比较两张票。它必须选一张：185 分钟相对 60 分钟规则多 125 分钟，因此是 `Likely comfortable`；是否有联票/行李直挂则独立显示为 `Ticket protection not confirmed`。未知保障不能被偷换成‘时间不够’。” |
| 1:05–1:20 | 展开 `How this judgment was made` | “这里展示 60 分钟公开规则加 90 分钟规划缓冲、模型、来源、耗时和缓存状态。它是可见的规划启发式，不是历史误机概率。” |
...
3. `Agent research` 中每条链接都要有“它支持什么”的摘要；无相关官方来源时应失败关闭，不给伪研究结论。
