...
User-recorded screen-capture video; this document is the shot-by-shot script plus a material pack index. Every fact below was checked against the current code and gates (`npm run verify` = 60 assertions, `npm run test` = 34 numeric cases, `npm run smoke:research`, `npm run smoke:server`). Narration is written in Chinese with an English reference translation per shot.

用户自行录屏的 3 分钟演示视频分镜脚本与素材包索引。下述所有事实均与当前代码和门禁核对（`npm run verify` = 60 条断言、`npm run test` = 34 个数值用例、`npm run smoke:research`、`npm run smoke:server`）。口播以中文撰写，每镜头附英文对照。
...
| Gate to quote 可引用门禁 | `npm run verify` 60 passed；`npm run test` 34 cases；`npm run smoke:research`（status=200、attempts=2、official 来源 ×1） |
...
| 5 | 30s | 点击 "Ask agent which itinerary to choose"，展示 live 证据链研究结果：来源列表（Official 来源 ×1 + Community 来源，各自可点击的原文链接）；展开 "How this judgment was made" 遥测面板，逐行扫过：Time-fit rubric（60 min published minimum + 90 min planning buffer (AirAsia Fly-Thru · KLIA Terminal 2)）、Workflow（Tavily evidence search → DeepSeek assessment）、Search sources、Search rounds（2 · round 2 reformulated the official query…）、Policy entry、Result origin。素材：`current-4-live-research.png` | 这是实时证据链研究：服务端用有界的 Tavily 检索——最多两轮，第一轮没有官方来源才改写查询重试；官方域名被白名单限定为 airasia.com。每条来源分层展示 official 或 community，全部带原文链接。整个遥测面板公开：跑了几轮、查了几个来源、用了哪条策略条目、结果来自实时运行还是 30 分钟缓存——没有任何黑盒。 | This is live evidence research: a bounded Tavily loop — at most two rounds, round 2 only reformulates when round 1 found no official source; official results are domain-gated to airasia.com. Every source is tiered official or community, each with its original link. The telemetry is public: rounds, sources, which policy entry ran, live run or 30-minute cache. No black box. | 遥测面板逐行下划线强调；字幕：bounded two-round search · official-domain gate |
...
| # | 时长 | 画面内容 | 口播稿（中文） | Narration (EN reference) | 转场/字幕建议 |
| --- | --- | --- | --- | --- | --- |
| 8 | 8s | 旅客侧点击 "Use recommended itinerary"（同意门），自动切到 "Airline: intervene after an event" 侧；Booked connection watch 显示所选行程与 DELAY TRIGGER 区（Scenario replay / Manual 两个触发方式）。素材：`current-5-airline-side.png` | 旅客确认后进入航司侧。注意同意门：没有旅客确认，行程不会进入后续流程。 | After traveller consent we switch to the airline side — note the consent gate: nothing proceeds without it. | 字幕：Consent required |
